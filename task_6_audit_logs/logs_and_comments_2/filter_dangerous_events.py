...
class DangerousEventFilter:
    ...
    def is_debug_command(self, event: Dict[str, Any]) -> bool:
        ...
        if (event.get('verb') == 'patch' and 
            '/ephemeralcontainers' in event.get('requestURI', '')):
    ...
    def is_rbac_escalation(self, event: Dict[str, Any]) -> bool:
        ...
        if (event.get('verb') == 'create' and 
            'rolebindings' in event.get('requestURI', '')):
    ...
    def is_secret_access(self, event: Dict[str, Any]) -> bool:
        ...
        if (event.get('verb') in ['list', 'get'] and 
            '/secrets' in request_uri):
    ...
    def is_namespace_creation(self, event: Dict[str, Any]) -> bool:
        ...
        if (event.get('verb') == 'create' and 
            event.get('objectRef', {}).get('resource') == 'namespaces'):
    ...
    def is_serviceaccount_creation(self, event: Dict[str, Any]) -> bool:
        ...
        if (event.get('verb') == 'create' and 
            event.get('objectRef', {}).get('resource') == 'serviceaccounts'):
    ...
    def print_summary(self):
        ...
        print(f"📈 Detection rate: {self.stats['dangerous_events']/self.stats['total_events']*100:.2f}%", file=sys.stderr)
...
Examples:
    ...
    parser.add_argument('audit_log', 
                       help='Path to audit log file (use "-" for stdin)')
    parser.add_argument('--summary-only', '-s', 
                       action='store_true',
                       help='Only show summary statistics, no event output')
    parser.add_argument('--threat-level', '-t',
                       choices=['CRITICAL', 'HIGH', 'MEDIUM'],
                       help='Filter by minimum threat level')
    ...
    try:
        ...
        if args.threat_level:
            filtered_events = [
                ...
                if event['_threat_level'] == args.threat_level or 
                   (args.threat_level == 'HIGH' and event['_threat_level'] == 'CRITICAL') or
                   (args.threat_level == 'MEDIUM' and event['_threat_level'] in ['HIGH', 'CRITICAL'])
            ]
