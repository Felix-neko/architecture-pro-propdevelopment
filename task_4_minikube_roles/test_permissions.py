...
def test_configmap_operations(v1_core: client.CoreV1Api, namespaces: List[str]) -> Dict[str, Dict[str, bool]]:
    ...
    for namespace in namespaces:
        results[namespace] = {"create_configmap": False, "read_configmap": False}

        # Тест создания ConfigMap
        try:
            configmap = client.V1ConfigMap(
                metadata=client.V1ObjectMeta(name="test-config"),
                data={"app.properties": "debug=true\nlog_level=info", "database.url": "localhost:5432"},
            )
...
def test_secret_operations(v1_core: client.CoreV1Api, namespaces: List[str]) -> Dict[str, Dict[str, bool]]:
    ...
    for namespace in namespaces:
        results[namespace] = {"create_secret": False, "list_secrets": False, "read_secret_data": False}
        ...
        try:
            ...
            secret = client.V1Secret(metadata=client.V1ObjectMeta(name="test-secret"), data=secret_data, type="Opaque")
...
def test_pod_operations(v1_core: client.CoreV1Api, namespaces: List[str]) -> Dict[str, Dict[str, bool]]:
    ...
    for namespace in namespaces:
        results[namespace] = {"list_pods": False, "create_pod": False, "read_pod_logs": False}
                ...
                spec=client.V1PodSpec(
                    containers=[
                        client.V1Container(
                            ...
                            env=[client.V1EnvVar(name="MESSAGE", value="It works!")],
                        )
                    ],
        ...
        if results[namespace]["list_pods"]:
            try:
                pods = v1_core.list_namespaced_pod(namespace=namespace)
                existing_pods = [
                    pod.metadata.name for pod in pods.items if pod.status.phase in ["Running", "Succeeded", "Failed"]
                ]
            except ApiException:
                pass
        ...
        for pod_name in existing_pods:
            try:
                ...
                logs = v1_core.read_namespaced_pod_log(
                    name=pod_name, namespace=namespace, tail_lines=10, previous=False, timestamps=False
                )
                if logs:
                ...
                if e.status == 400:  # Bad Request
                    try:
                        logs = v1_core.read_namespaced_pod_log(name=pod_name, namespace=namespace, previous=True)
...
def test_service_operations(v1_core: client.CoreV1Api, namespaces: List[str]) -> Dict[str, Dict[str, bool]]:
    ...
    for namespace in namespaces:
        results[namespace] = {"list_services": False, "create_service": False}
        ...
        try:
            service = client.V1Service(
                ...
                spec=client.V1ServiceSpec(
                    ...
                    ports=[client.V1ServicePort(name="http", port=80, target_port=8080, protocol="TCP")],
                    type="ClusterIP",
                ),
            )
            ...
            try:
                ...
                if e.status == 409:  # Already exists
                    ...
                    v1_core.patch_namespaced_service(name="test-service", namespace=namespace, body=service)
...
def print_results_table(
    namespaces: List[str], configmap_results: Dict, secret_results: Dict, pod_results: Dict, service_results: Dict
):
    """Выводит итоговую таблицу результатов используя tabulate"""
    ...
    for namespace in namespaces:
        ...
        # Применяем цвета к символам
        cm_create_colored = (
            f"{Colors.GREEN}{cm_create}{Colors.END}" if cm_create == "✓" else f"{Colors.RED}{cm_create}{Colors.END}"
        )
        cm_read_colored = (
            f"{Colors.GREEN}{cm_read}{Colors.END}" if cm_read == "✓" else f"{Colors.RED}{cm_read}{Colors.END}"
        )
        sec_create_colored = (
            f"{Colors.GREEN}{sec_create}{Colors.END}" if sec_create == "✓" else f"{Colors.RED}{sec_create}{Colors.END}"
        )
        sec_list_colored = (
            f"{Colors.GREEN}{sec_list}{Colors.END}" if sec_list == "✓" else f"{Colors.RED}{sec_list}{Colors.END}"
        )
        sec_read_colored = (
            f"{Colors.GREEN}{sec_read}{Colors.END}" if sec_read == "✓" else f"{Colors.RED}{sec_read}{Colors.END}"
        )
        pod_list_colored = (
            f"{Colors.GREEN}{pod_list}{Colors.END}" if pod_list == "✓" else f"{Colors.RED}{pod_list}{Colors.END}"
        )
        pod_create_colored = (
            f"{Colors.GREEN}{pod_create}{Colors.END}" if pod_create == "✓" else f"{Colors.RED}{pod_create}{Colors.END}"
        )
        pod_logs_colored = (
            f"{Colors.GREEN}{pod_logs}{Colors.END}" if pod_logs == "✓" else f"{Colors.RED}{pod_logs}{Colors.END}"
        )
        svc_list_colored = (
            f"{Colors.GREEN}{svc_list}{Colors.END}" if svc_list == "✓" else f"{Colors.RED}{svc_list}{Colors.END}"
        )
        svc_create_colored = (
            f"{Colors.GREEN}{svc_create}{Colors.END}" if svc_create == "✓" else f"{Colors.RED}{svc_create}{Colors.END}"
        )
...
Примеры использования:
  ...
  python3 test_permissions.py --kubeconfig kubeconfig_anton  # Использовать конкретный kubeconfig
    ...
    parser.add_argument(
        "--kubeconfig", type=str, help="Путь к kubeconfig файлу (по умолчанию используется ~/.kube/config)"
    )
