...
rm -f $USERS_DIR/${USERNAME}.key $USERS_DIR/${USERNAME}.csr $USERS_DIR/${USERNAME}.crt $USERS_DIR/ca.crt $USERS_DIR/kubeconfig_${USERNAME}
...
openssl req -new -key $USERS_DIR/${USERNAME}.key -out $USERS_DIR/${USERNAME}.csr -subj "$SUBJECT"

# Создаем временный YAML для CSR
cat > $USERS_DIR/user-${USERNAME}.yaml << EOF
...
CSR_B64=$(cat $USERS_DIR/${USERNAME}.csr | base64 | tr -d '\n')
sed "s/\${CSR_B64}/$CSR_B64/g" $USERS_DIR/user-${USERNAME}.yaml | kubectl apply -f -
...
kubectl get csr ${USERNAME}-csr -o jsonpath='{.status.certificate}' | base64 --decode > $USERS_DIR/${USERNAME}.crt

# Создаем отдельный kubeconfig для пользователя
KUBECONFIG_USER=$USERS_DIR/kubeconfig_${USERNAME}
...
if [ -z "$CA_CERT_PATH" ]; then
    ...
    kubectl config view --raw --minify --flatten -o jsonpath='{.clusters[0].cluster.certificate-authority-data}' | base64 --decode > $USERS_DIR/ca.crt
    CA_CERT_PATH=$USERS_DIR/ca.crt
fi
...
kubectl config --kubeconfig=$KUBECONFIG_USER set-credentials $USERNAME --client-certificate=$USERS_DIR/${USERNAME}.crt --client-key=$USERS_DIR/${USERNAME}.key --embed-certs=true
...
rm -f $USERS_DIR/user-${USERNAME}.yaml
